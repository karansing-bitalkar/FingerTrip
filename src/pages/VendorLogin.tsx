...
import {
  Eye, EyeOff, Building2, ArrowLeft, LogIn,
  Globe, Package, DollarSign, Star, Users,
...
const VENDOR_USER: User & { password: string } = {
  id: '2',
...
export default function VendorLogin() {
  ...
  const handleDemoClick = () => {
    setForm({ email: VENDOR_USER.email, password: VENDOR_USER.password });
    setError('');
    setDemoFilled(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setLoading(true);
    setTimeout(() => {
      if (form.email !== VENDOR_USER.email || form.password !== VENDOR_USER.password) {
        setError('Invalid vendor credentials. Use the demo card above.');
        setLoading(false);
        return;
      }
      const { password: _, ...userData } = VENDOR_USER;
      storeUser(userData);
      setLoading(false);
  ...
  return (
    <div className="min-h-screen flex bg-[#f0fafb]">
        ...
        <div className="relative z-10 flex flex-col h-full p-12">
          ...
          <Link to="/" className="flex items-center gap-2.5 text-white w-fit">
            ...
            <span className="text-xl font-bold font-display tracking-tight">
              FingerTrip <span className="text-[#AFDDE5]">Vendors</span>
            </span>
          </Link>
          ...
          <div className="flex-1 flex flex-col justify-center max-w-sm">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              ...
              <p className="text-[#AFDDE5]/80 text-base leading-relaxed mb-10">
                List packages, manage bookings, track real-time earnings, and reach 150,000+ eager travelers worldwide.
              </p>

              <div className="grid grid-cols-2 gap-3 mb-8">
                {[
                  { icon: <Package className="w-5 h-5" />, label: 'List Packages', desc: 'Unlimited listings' },
                  { icon: <DollarSign className="w-5 h-5" />, label: 'Track Earnings', desc: 'Real-time payouts' },
                  { icon: <Star className="w-5 h-5" />, label: 'Build Reviews', desc: 'Grow reputation' },
                  { icon: <Users className="w-5 h-5" />, label: '150K+ Travelers', desc: 'Global reach' },
                ].map((item) => (
                  <div
                    key={item.label}
                    className="bg-white/10 backdrop-blur-sm border border-white/15 rounded-2xl px-4 py-3 flex items-start gap-3"
                  >
                    <div className="mt-0.5 text-[#AFDDE5]">{item.icon}</div>
                    <div>
                      <div className="font-bold text-white text-sm">{item.label}</div>
                      <div className="text-[#AFDDE5]/70 text-xs">{item.desc}</div>
                    </div>
                  </div>
                ))}
              </div>

              <div className="flex items-center gap-2 text-[#AFDDE5]/70 text-sm">
                <TrendingUp className="w-4 h-4 text-[#0FA4AF]" />
                <span>Avg vendor earns <span className="text-white font-semibold">$4,200/mo</span> on FingerTrip</span>
              </div>
            </motion.div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 28 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-[420px]">
          ...
          <div className="lg:hidden flex items-center gap-2 mb-8">
            ...
            <span className="text-xl font-bold font-display text-[#003135]">
              Finger<span className="text-[#0FA4AF]">Trip</span> Vendors
            </span>
          </div>

          <h1 className="text-3xl font-bold text-[#003135] font-display mb-1">Vendor Portal</h1>
          <p className="text-gray-400 text-sm mb-8">Sign in to manage your travel business</p>
          ...
          <div className="mb-7">
            ...
            <button
              ...
              className={`w-full p-4 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white text-left hover:scale-[1.02] hover:shadow-lg hover:shadow-emerald-200 transition-all duration-200 shadow-sm relative overflow-hidden`}
            >
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm">Sunrise Travels Co.</div>
                  <div className="text-xs opacity-80 mt-0.5">vendor@fingertrip.com · Vendor@123</div>
                  <div className="text-[11px] opacity-60 mt-1">Click to auto-fill credentials</div>
                </div>
                {demofilled && (
            ...
            {demofilled && (
              <p className="text-xs text-emerald-600 text-center mt-2 font-medium">
                ✓ Credentials loaded — click Sign In below
              </p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-gray-300 font-medium">or sign in manually</span>
            <div className="flex-1 h-px bg-gray-100" />
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-500 uppercase mb-1.5 block tracking-wide">Password</label>
              ...
              <div className="flex justify-end mt-1.5">
                <Link to="/forgot-password" className="text-xs text-[#0FA4AF] hover:text-[#024950] font-medium transition-colors">
                  Forgot password?
                </Link>
              </div>
            </div>
          ...
          <div className="mt-6 text-center space-y-2">
            ...
            <p className="text-xs text-gray-400">
              New vendor?{' '}
              <Link to="/register" className="text-[#964734] font-medium hover:text-[#003135] transition-colors">
                Apply to join FingerTrip
              </Link>
            </p>
            <Link to="/admin-login" className="text-xs text-gray-300 hover:text-[#0FA4AF] transition-colors block">
              Admin Login
            </Link>
          </div>
        </motion.div>
      ...
      <Popup
        isOpen={successPopup}
        onClose={() => { setSuccessPopup(false); navigate('/vendor-dashboard'); }}
        type="success"
        title="Welcome back!"
        message="Signed in as Sunrise Travels Co. Redirecting to your Vendor Dashboard..."
      />
    </div>
  );
}
